...
def run_this(RL_set, n_episode, n_agents, n_actions, vector_obs_len, gamma, save_model_freq):
    step = 0
    ...
    for episode in range(n_episode):
        ...
        while True:
            ...
            for agent_id in range(n_agents):
                action_output = RL_set[agent_id][0].predict(observation_set[agent_id])[0]
                action_output_set.append(action_output)
                action_prob = action_output
                action_to_choose = np.random.choice(action_list, p=action_prob)
                action_one_hot = np.zeros([1, n_actions])
                action_one_hot[0, action_to_choose] = 1
            ...
            for agent_id in range(n_agents):
                ...
                state_value = critic.predict(observation_set[agent_id])[0]
                next_state_value = critic.predict(observation_set_next[agent_id])[0]
                if done == True:
                ...
                actor.train(observation_set[agent_id], action_one_hot_list[agent_id], advantage)
                critic.train(observation_set[agent_id], target_q_value)

                episode_reward_agent[agent_id] += reward
                RL_set[agent_id][1].get_episode_reward(episode_reward_agent[agent_id], episode_reward_all, episode)
                if (step > 0 and step % save_model_freq == 0):
                    actor.save_model(step)
                    print("Model have been trained for %s times" % (step))
...
if __name__ == "__main__":
    ...
    for i in range(n_agents):
        ...
        with sess.as_default():
            with g.as_default():
                net_set = []
                actor = ActorNetwork(sess, learning_rate_actor, n_features, n_actions, i)
                critic = CriticNetwork(sess, learning_rate_critic, n_features, output_len, n_actions, i)
                if (load_model):
    ...
    run_this(agent_set, n_episode, n_agents, n_actions, vector_obs_len, reward_decay, save_model_freq)